...
configured with logging capabilities, so that STDOUT and STDERR of the
services it has the responsibities of aren't lost.
...
to make a service restart automatically once it finished. But note that
...
and by mean of fork-and-exec share that sockets among multiple services
...
of your webcluster. You just have to declare in Perlbal's nodefile one
